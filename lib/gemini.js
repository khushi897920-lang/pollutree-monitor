...
Provide a 2-sentence health advisory for citizens. Be concise and actionable.`;
    ...
    if (msg.includes('API key')) {
      return `⚠️ [System Alert]: Your Gemini API key is invalid or expired. Please update it in your .env.local file to receive live AI advisories.`;
    }
...
Provide a helpful, concise answer based on the AQI data. If the question is about health impacts, provide specific advice. Keep the response under 100 words.`;
    ...
    if (msg.includes('API key')) {
      return `[System Alert]: Your Gemini API key is strictly invalid. Update .env.local to restore AI chat.`;
    }
...
export async function generateForecast(aqiData) {
  try {
    const model = getGeminiModel();
    const aqi_score = aqiData.aqi_score || 0;
    const pm25 = aqiData.pm25 || aqiData.pm25_level || 0;
    const ward_name = aqiData.ward_name || '';

    const prompt = `Current air quality: AQI ${aqi_score}, PM2.5 ${pm25}, ward: ${ward_name}.
Return ONLY a valid JSON array. No markdown, no backticks, no explanation.
[{time:'1PM',aqi:180,label:'Unhealthy'},
...6 items total]`;
